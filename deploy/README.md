...
- CI publishes OCI charts to `oci://ghcr.io/<owner>/kubenova/charts/{manager,operator}`
  ...
  - release tags (e.g., `v0.1.3`) mirror git tags

Install (OCI)
```bash
helm registry login ghcr.io -u <user> -p <token>
helm pull oci://ghcr.io/<owner>/kubenova/charts/manager --version v0.1.3
helm pull oci://ghcr.io/<owner>/kubenova/charts/operator --version v0.1.3
```
