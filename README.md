...
- Branch/tag mapping
  - develop: chart version is suffixed with -dev (e.g., 0.9.5-dev). A lightweight OCI tag alias dev also points to the same artifact.
  - main: chart version is the normal semver (e.g., 0.9.5). A lightweight OCI tag alias latest also points to the same artifact.
...
helm pull oci://ghcr.io/vaheed/kubenova-charts/manager --version v0.9.5   # alias to the same 0.9.5 artifact
```
```
