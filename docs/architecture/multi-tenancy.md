...
- Two kubeconfigs per tenant derived from the ServiceAccounts (Secret `kubenova-kubeconfigs` in `<tenant>-owner`; Manager API surfaces these). The kubeconfigs point to the cluster Capsule Proxy endpoint (no tenant/role path), never the raw kube-apiserver.
