...
- Tenant kubeconfigs: `GET /tenants/{tenantId}/kubeconfig` returns inline owner/readonly kubeconfigs pointing at the Capsule Proxy base when the Secret exists; before the Secret is written, it may return the proxy base URL. Always register clusters with `capsuleProxyEndpoint` so tenants route through the proxy.
