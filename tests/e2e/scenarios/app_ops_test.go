...
func TestAppOperations(t *testing.T) {
    ...
    if env == nil { t.Skip("suite environment unavailable") }

    // Ensure cluster registered
    info, err := env.EnsureClusterRegistered(t.Context(), env.Config().ClusterName)
    if err != nil { t.Fatalf("cluster registration failed: %v", err) }
...
func doStatus(t *testing.T, c *http.Client, method, url string, body *bytes.Reader, want int) {
    ...
    if body != nil { rdr = body } else { rdr = bytes.NewReader(nil) }
    req, _ := http.NewRequest(method, url, rdr)
    req.Header.Set("Content-Type", "application/json")
    resp, err := c.Do(req)
    if err != nil { t.Fatalf("%s %s: %v", method, url, err) }
    defer resp.Body.Close()
    if resp.StatusCode != want { t.Fatalf("%s %s: got %d want %d", method, url, resp.StatusCode, want) }
}
