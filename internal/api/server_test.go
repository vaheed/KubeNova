...
func TestTenantsCRUD(t *testing.T) {
    ...
    if w.Code != 200 { t.Fatalf("create tenant failed: %d", w.Code) }
    // list
    req = httptest.NewRequest(http.MethodGet, "/api/v1/tenants", nil)
    w = httptest.NewRecorder()
    s.Router().ServeHTTP(w, req)
    if w.Code != 200 { t.Fatalf("list tenant failed: %d", w.Code) }
}
...
func TestRBACFiltersTenants(t *testing.T) {
    ...
    if w.Code != 200 { t.Fatalf("rbac list failed: %d", w.Code) }
...
func TestCreateClusterEndpoint(t *testing.T) {
    ...
    w := httptest.NewRecorder()
    srv.Router().ServeHTTP(w, req)
    if w.Code != 200 { t.Fatalf("cluster create failed: %d %s", w.Code, w.Body.String()) }
}
...
func TestIngestEventsAndList(t *testing.T) {
    ...
    w := httptest.NewRecorder()
    srv.Router().ServeHTTP(w, req)
    if w.Code != http.StatusNoContent { t.Fatalf("ingest failed: %d", w.Code) }
    items, err := st.ListClusterEvents(context.Background(), 1, 10)
    if err != nil || len(items) != 1 { t.Fatalf("list events: %v %d", err, len(items)) }
}
