...
func TestAdditionalEndpointsCoverage(t *testing.T) {
	...
	resp, err = http.Post(ts.URL+"/api/v1/clusters/"+idStr(cl.Id)+"/bootstrap/tenancy", "application/json", nil)
	if err != nil {
	...
	// 2) PolicySet catalog
	resp, err = http.Get(ts.URL + "/api/v1/clusters/" + idStr(cl.Id) + "/policysets/catalog")
	if err != nil {
	...
	rr.Body.Close()
	req, _ := http.NewRequest(http.MethodPut, ts.URL+"/api/v1/clusters/"+idStr(c.Id)+"/tenants/"+idStr(tnt.Id)+"/owners", bytes.NewReader(owners))
	...
	// 4) Tenant summary
	resp, err = http.Get(ts.URL + "/api/v1/clusters/" + idStr(c.Id) + "/tenants/" + idStr(tnt.Id) + "/summary")
	if err != nil {
	...
	rr.Body.Close()
	req, _ = http.NewRequest(http.MethodPut, ts.URL+"/api/v1/clusters/"+idStr(c.Id)+"/tenants/"+idStr(tnt.Id)+"/projects/"+idStr(pr.Id)+"/access", bytes.NewReader([]byte(`{"members":[]}`)))
	req.Header.Set("Content-Type", "application/json")
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
	...
	rr.Body.Close()
	resp, err = http.Post(ts.URL+"/api/v1/clusters/"+idStr(c.Id)+"/tenants/"+idStr(tnt.Id)+"/projects/"+idStr(pr.Id)+"/apps/"+idStr(ap.Id)+":delete", "application/json", nil)
	if err != nil {
	...
	// 7) Project usage
	resp, err = http.Get(ts.URL + "/api/v1/projects/" + idStr(pr.Id) + "/usage")
	if err != nil {
	...
	// 8) Tenant usage
	resp, err = http.Get(ts.URL + "/api/v1/tenants/" + idStr(tnt.Id) + "/usage")
	if err != nil {
