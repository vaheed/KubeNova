...
func TestProjectsAndAppsLifecycle(t *testing.T) {
    ...
    if resp.StatusCode != http.StatusOK { t.Fatalf("create tenant: %s", resp.Status) }
    resp.Body.Close()
    ...
    resp, err = http.DefaultClient.Do(req)
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("create project: %s", resp.Status) }
    resp.Body.Close()

    // list projects
    resp, err = http.Get(ts.URL+"/api/v1/clusters/c/tenants/acme/projects")
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("list projects: %s", resp.Status) }
    var projects []Project
    _ = json.NewDecoder(resp.Body).Decode(&projects)
    resp.Body.Close()
    if len(projects) != 1 || projects[0].Name != "web" { t.Fatalf("unexpected projects: %+v", projects) }
    ...
    resp, err = http.DefaultClient.Do(req)
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("create app: %s", resp.Status) }
    resp.Body.Close()

    // list apps
    resp, err = http.Get(ts.URL+"/api/v1/clusters/c/tenants/acme/projects/web/apps")
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("list apps: %s", resp.Status) }
    var apps []App
    _ = json.NewDecoder(resp.Body).Decode(&apps)
    resp.Body.Close()
    if len(apps) != 1 || apps[0].Name != "hello" { t.Fatalf("unexpected apps: %+v", apps) }

    // get app
    resp, err = http.Get(ts.URL+"/api/v1/clusters/c/tenants/acme/projects/web/apps/hello")
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusOK { t.Fatalf("get app: %s", resp.Status) }
    resp.Body.Close()
    ...
    resp, err = http.DefaultClient.Do(req)
    if err != nil { t.Fatal(err) }
    if resp.StatusCode != http.StatusAccepted { t.Fatalf("delete app: %s", resp.Status) }
    resp.Body.Close()
}
