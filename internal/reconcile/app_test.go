...
func TestAppReconcilerCreatesVelaApplication(t *testing.T) {
    ...
    if err := c.Create(context.Background(), cm); err != nil { t.Fatal(err) }

    if _, err := r.Reconcile(context.Background(), ctrl.Request{NamespacedName: types.NamespacedName{Namespace: cm.Namespace, Name: cm.Name}}); err != nil { t.Fatal(err) }
