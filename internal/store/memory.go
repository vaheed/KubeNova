...
func (m *Memory) CreateTenant(ctx context.Context, t types.Tenant) error {
    ...
    if _, ok := m.tenants[t.Name]; ok { return nil }
...
func (m *Memory) GetTenant(ctx context.Context, name string) (types.Tenant, error) {
    ...
    t, ok := m.tenants[name]; if !ok { return types.Tenant{}, ErrNotFound }
    return t, nil
}
func (m *Memory) ListTenants(ctx context.Context) ([]types.Tenant, error) {
    m.mu.RLock(); defer m.mu.RUnlock()
    out := make([]types.Tenant, 0, len(m.tenants))
    for _, t := range m.tenants { out = append(out, t) }
    return out, nil
}
func (m *Memory) UpdateTenant(ctx context.Context, t types.Tenant) error { return m.CreateTenant(ctx, t) }
...
func (m *Memory) CreateProject(ctx context.Context, p types.Project) error {
    m.mu.Lock(); defer m.mu.Unlock()
    if _, ok := m.projects[p.Tenant]; !ok { m.projects[p.Tenant] = map[string]types.Project{} }
    if _, ok := m.projects[p.Tenant][p.Name]; ok { return nil }
...
func (m *Memory) GetProject(ctx context.Context, tenant, name string) (types.Project, error) {
    ...
    if mp, ok := m.projects[tenant]; ok {
        if p, ok2 := mp[name]; ok2 { return p, nil }
    }
    return types.Project{}, ErrNotFound
}
func (m *Memory) ListProjects(ctx context.Context, tenant string) ([]types.Project, error) {
    m.mu.RLock(); defer m.mu.RUnlock()
    mp, ok := m.projects[tenant]; if !ok { return []types.Project{}, nil }
    out := make([]types.Project, 0, len(mp))
    for _, p := range mp { out = append(out, p) }
    return out, nil
}
func (m *Memory) UpdateProject(ctx context.Context, p types.Project) error { return m.CreateProject(ctx, p) }
func (m *Memory) DeleteProject(ctx context.Context, tenant, name string) error {
    m.mu.Lock(); defer m.mu.Unlock()
    if mp, ok := m.projects[tenant]; ok { delete(mp, name) }
    if ma, ok := m.apps[tenant]; ok { delete(ma, name) }
    return nil
}

func (m *Memory) CreateApp(ctx context.Context, a types.App) error {
    m.mu.Lock(); defer m.mu.Unlock()
    if _, ok := m.apps[a.Tenant]; !ok { m.apps[a.Tenant] = map[string]map[string]types.App{} }
    if _, ok := m.apps[a.Tenant][a.Project]; !ok { m.apps[a.Tenant][a.Project] = map[string]types.App{} }
    if _, ok := m.apps[a.Tenant][a.Project][a.Name]; ok { return nil }
...
func (m *Memory) GetApp(ctx context.Context, tenant, project, name string) (types.App, error) {
    m.mu.RLock(); defer m.mu.RUnlock()
    ma, ok := m.apps[tenant]; if !ok { return types.App{}, ErrNotFound }
    mp, ok := ma[project]; if !ok { return types.App{}, ErrNotFound }
    a, ok := mp[name]; if !ok { return types.App{}, ErrNotFound }
    return a, nil
}
func (m *Memory) ListApps(ctx context.Context, tenant, project string) ([]types.App, error) {
    m.mu.RLock(); defer m.mu.RUnlock()
    ma, ok := m.apps[tenant]; if !ok { return []types.App{}, nil }
    mp, ok := ma[project]; if !ok { return []types.App{}, nil }
    out := make([]types.App, 0, len(mp))
    for _, a := range mp { out = append(out, a) }
    return out, nil
}
...
func (m *Memory) DeleteApp(ctx context.Context, tenant, project, name string) error {
    ...
    if ma, ok := m.apps[tenant]; ok {
        if mp, ok2 := ma[project]; ok2 { delete(mp, name) }
    }
    return nil
}
...
func (m *Memory) GetCluster(ctx context.Context, id int) (types.Cluster, string, error) {
    m.mu.RLock(); defer m.mu.RUnlock()
    mc, ok := m.clusters[id]
    if !ok { return types.Cluster{}, "", ErrNotFound }
...
func (m *Memory) AddEvents(ctx context.Context, clusterID *int, evts []types.Event) error {
    m.mu.Lock(); defer m.mu.Unlock()
    for _, e := range evts { m.evts = append(m.evts, memEvent{clusterID: clusterID, e: e}) }
    return nil
}

func (m *Memory) AddConditionHistory(ctx context.Context, clusterID int, conds []types.Condition) error { return nil }
...
func (m *Memory) ListClusterEvents(ctx context.Context, clusterID int, limit int) ([]types.Event, error) {
    ...
    for i := len(m.evts)-1; i >=0 && len(out) < limit; i-- {
        me := m.evts[i]
        if me.clusterID != nil && *me.clusterID == clusterID { out = append(out, me.e) }
    }
    return out, nil
}
