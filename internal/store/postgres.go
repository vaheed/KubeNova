...
func NewPostgres(ctx context.Context, url string) (*Postgres, error) {
    ...
    if err != nil { return nil, err }
    p := &Postgres{db: db}
    if err := p.applyMigration(ctx); err != nil { db.Close(); return nil, err }
    return p, nil
}
...
func (p *Postgres) GetTenant(ctx context.Context, name string) (types.Tenant, error) {
    ...
    if err != nil { return types.Tenant{}, ErrNotFound }
    return t, nil
}
func (p *Postgres) ListTenants(ctx context.Context) ([]types.Tenant, error) {
    ...
    for rows.Next() { var t types.Tenant; if err := rows.Scan(&t.Name, &t.CreatedAt); err != nil { return nil, err }; out = append(out, t) }
    return out, nil
}
func (p *Postgres) UpdateTenant(ctx context.Context, t types.Tenant) error { return p.CreateTenant(ctx, t) }
...
func (p *Postgres) GetProject(ctx context.Context, tenant, name string) (types.Project, error) {
    ...
    if err != nil { return types.Project{}, ErrNotFound }
    return pr, nil
}
func (p *Postgres) ListProjects(ctx context.Context, tenant string) ([]types.Project, error) {
    ...
    for rows.Next() { var pr types.Project; if err := rows.Scan(&pr.Tenant, &pr.Name); err != nil { return nil, err }; out = append(out, pr) }
    return out, nil
}
func (p *Postgres) UpdateProject(ctx context.Context, pr types.Project) error { return p.CreateProject(ctx, pr) }
...
func (p *Postgres) GetApp(ctx context.Context, tenant, project, name string) (types.App, error) {
    ...
    if err != nil { return types.App{}, ErrNotFound }
    return a, nil
}
func (p *Postgres) ListApps(ctx context.Context, tenant, project string) ([]types.App, error) {
    ...
    defer rows.Close()
    var out []types.App
    for rows.Next() { var a types.App; if err := rows.Scan(&a.Tenant, &a.Project, &a.Name); err != nil { return nil, err }; out = append(out, a) }
    return out, nil
}
...
func (p *Postgres) GetCluster(ctx context.Context, id int) (types.Cluster, string, error) {
    ...
    if err != nil { return types.Cluster{}, "", ErrNotFound }
    c.ID = id
    c.Labels = labels
    return c, enc, nil
}
...
func (p *Postgres) AddEvents(ctx context.Context, clusterID *int, evts []types.Event) error {
    for _, e := range evts {
        var cid any = nil
        if clusterID != nil { cid = *clusterID }
...
func (p *Postgres) ListClusterEvents(ctx context.Context, clusterID int, limit int) ([]types.Event, error) {
    ...
    for rows.Next() {
        var e types.Event
        if err := rows.Scan(&e.Type, &e.Resource, &e.Payload, &e.TS); err != nil { return nil, err }
        out = append(out, e)
    }
    return out, nil
}
