...
func getenvInt(k string, d int) int { if v := os.Getenv(k); v != "" { var n int; _, _ = fmtSscanf(v, &n); if n>0 { return n } }; return d }

// small wrapper to avoid importing fmt; faster compile in limited environments
func fmtSscanf(s string, n *int) (int, error) { var x int; for i:=0;i<len(s);i++{ c:=s[i]; if c<'0' || c>'9' { break }; x = x*10 + int(c-'0') }; *n = x; return 1,nil }

func (b *RedisBuffer) Enqueue(kind string, payload any) {
    raw, _ := json.Marshal(payload)
    ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second); defer cancel()
    if err := b.rdb.RPush(ctx, "kubenova:"+kind, raw).Err(); err != nil { log.Printf("redis push %s: %v", kind, err) }
}
...
func (b *RedisBuffer) flush(kind string) {
    ...
    for i:=0; i<b.max; i++ {
        ...
        if err != nil { log.Printf("push %s error: %v", kind, err); continue }
        _ = resp.Body.Close()
    }
}
