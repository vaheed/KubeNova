...
func main() {
    ...
    if err != nil { logging.L.Fatal("manager", zap.Error(err)) }
    ...
    if err := mgr.AddHealthzCheck("healthz", healthz.Ping); err != nil { logging.L.Fatal("healthz", zap.Error(err)) }
    if err := mgr.AddReadyzCheck("readyz", healthz.Ping); err != nil { logging.L.Fatal("readyz", zap.Error(err)) }
    ...
    go func(){
        if err := reconcile.BootstrapHelmJob(ctrl.SetupSignalHandler()); err != nil { logging.L.Error("bootstrap error", zap.Error(err)) }
    }()

    logging.L.Info("KubeNova Agent starting manager")
    if err := mgr.Start(ctrl.SetupSignalHandler()); err != nil { logging.L.Fatal("manager stopped", zap.Error(err)) }
}
